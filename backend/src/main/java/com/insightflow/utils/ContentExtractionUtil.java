...
/**
 * Utility class for content extraction, parsing, and filtering from LinkedIn pages.
...
public class ContentExtractionUtil {
    ...
    private String extractCompanyTitle(WebDriver driver) {
        try {
            ...
            titleElements = driver.findElements(By.xpath("//h1[contains(@class, 'title') or contains(@class, 'summary')]"));
            if (!titleElements.isEmpty()) {
                String title = titleElements.get(0).getText().trim();
    ...
    private List<String> extractCompanyPosts(WebDriver driver, String companyName, String companyTitle) {
        ...
        try {
            // Strategy 1: Primary post selector
            ...
            if (!posts.isEmpty()) {
                logger.debug("Extracted {} posts with primary selector", posts.size());
            ...
            if (!posts.isEmpty()) {
                logger.debug("Extracted {} posts with Jsoup primary fallback", posts.size());
                return posts.stream().distinct().collect(Collectors.toList());
            }
            ...
            logger.debug("Used Jsoup alternative selectors, found {} posts", posts.size());
            return posts.stream().distinct().collect(Collectors.toList());
