...
import com.insightflow.services.MonitoredCompanyService;
...
public class SentimentScheduler {
    ...
    @Autowired
    private MonitoredCompanyService monitoredCompanyService;
    ...
    public void collectDailySentiment() {
        List<String> monitoredCompanies = monitoredCompanyService.getActiveCompanyNames();
        logger.info("Starting daily sentiment collection for companies: {}", monitoredCompanies);

        // Create a copy to avoid ConcurrentModificationException if list changes during
        // iteration
            ...
            try {
                ...
                for (Map<String, Object> dataPoint : scoredData) {
                    ...
                    if (!alreadyExists) {
                        ...
                        Map<String, Object> metadata = (Map<String, Object>) dataPoint.get("metadata");
    ...
    public List<String> getMonitoredCompanies() {
        return monitoredCompanyService.getActiveCompanyNames();
    }

    // Add a company to the monitoring list
    public boolean addCompany(String company) {
        boolean added = monitoredCompanyService.addCompany(company);
        if (added) {
    ...
    public boolean removeCompany(String company) {
        boolean removed = monitoredCompanyService.removeCompany(company);
        if (removed) {
    ...
    public boolean skipCompanyNextRun(String company) {
        if (monitoredCompanyService.isCompanyMonitored(company)) {
    ...
    public void setMonitoredCompanies(List<String> companies) {
        monitoredCompanyService.replaceMonitoredCompanies(companies);
        List<String> updatedList = monitoredCompanyService.getActiveCompanyNames();
        logger.info("Updated monitored companies list: {}", updatedList);
    }

    // Test method with custom companies
    public void testWithCompanies(List<String> companies, List<String> sources) {
        if (companies == null || companies.isEmpty()) {
            companies = monitoredCompanyService.getActiveCompanyNames();
        }
